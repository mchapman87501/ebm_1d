...
class EarthGeom:
    def __init__(self, num_zones: int) -> None:
        ...
        # Assume earth is a ... cylinder?!  Get the fraction of
        # the surface covered by each latitude zone.
        self.lats_frac = np.sin(self.lats_rad + delta_rad) - np.sin(
        ...
        self.lats_radius = np.cos(self.lats_rad)
        self.lats_height = np.sin(self.lats_rad + delta_rad) - np.sin(
            self.lats_rad - delta_rad
        )
        lats_area = self.lats_radius * self.lats_height  # * 2.0 * np.pi
